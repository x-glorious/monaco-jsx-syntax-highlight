...
export const disposeJsxElementOrFragment = (data: Data) => {
  ...
  if (node.kind === Typescript.SyntaxKind.JsxSelfClosingElement) {
    ...
    const openingNode = node.kind === Typescript.SyntaxKind.JsxFragment ? (node as any).openingFragment : (node as any).openingElement
    const closingNode = node.kind === Typescript.SyntaxKind.JsxFragment ? (node as any).closingFragment : (node as any).closingElement
    ...
    if(node.kind === Typescript.SyntaxKind.JsxElement){
      const { positions: openingTagNamePositions } = calcPosition((openingNode as any).tagName, lines)
      const { positions: closingTagNamePositions } = calcPosition((closingNode as any).tagName, lines)
      classifications.push({
